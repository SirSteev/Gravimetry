...
namespace PowerGridInventory
{
    ...
    public class PGIView : MonoBehaviour
    {
        ...
        PGISlot MakeSlot(int x, int y)
        {
            ...
            slot.OnClick.AddListener(OnClickHack);
        ...
        void OnClickHack(PointerEventData eventData, PGISlot slot)
        {
            if (eventData.button == PointerEventData.InputButton.Right && DraggedItem != null && slot != null)
            {
                ...
                // fix scale thing
                Debug.Log("Fix scale thing in PGIView.cs and find where drop happens and limit it to left click");

                SetDragIcon(DragIcon, DraggedItem, slot);

                DeselectAllViews();
                AppendClearList(slot.View);
                if (slot.View != null) slot.View.SelectSlot(slot, DraggedItem.Item);
            }
        }
